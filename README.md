...
- **결측 코드는 `-1`(비해당, 전체 셀의 89.6%)과 `-8`(모름·무응답, 0.2%)
  둘뿐이다.** 값 라벨에 등재된 `8`·`88`·`9`·`99`는 통합 이전 회차의
  잔여물이며, 실제 출현하는 8·9는 대부분 실질 응답이므로(예: `ATTEND` 8 =
  "전혀 가지 않는다") 코드값이 아니라 선택지의 연속 구간으로 판정해야 한다.
- KGSS는 패널이 아닌 반복 횡단면. 개인 수준 분석은 단일 회차 내에서만 가능
- `FINALWT`는 회차 내 정규화 가중치. 연도 통합 가중 분석 불가
- 층화·집락 변수 미공개 → 설계효과 반영 불가, 신뢰구간이 실제보다 좁음
- 오픈웨이트 모델 로드 시 `transformers` 버전뿐 아니라 `trust_remote_code`의
  원격 코드 리비전(`--revision`)도 함께 고정해야 재현 가능하다

자세한 프로젝트 컨텍스트 · 과거 함정 · 설계 결정은 `CLAUDE.md`(Claude
Code용) / `AGENTS.md`(Codex용)를 참고.
